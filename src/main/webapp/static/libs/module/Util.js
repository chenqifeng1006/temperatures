...
define([
    ...
    function () {
        ...
        Util.prototype.getCharLength = function(str){
            var str_length = 0;
         ...
         */
        Util.prototype.calculationTimeByMillisecond = function(millisecond, notZerofill){
            //共多少秒
            ...
            //所有的天
            var timeDistanceDay = Math.round((timeDistanceHour - hours) / 24);
            var days = timeDistanceDay % 30;

            //所有的月
            var months = Math.round((timeDistanceDay - days) / 30);
        ...
        /**
         * 将"millisecond"秒核算成b小时c分钟d秒e毫秒
         */
        Util.prototype.calculationTimeByMiniSeconds = function(millisecond, notZerofill){
            //共多少豪秒
            var millisecond = Math.round(millisecond/100)
         ...
         * 计算时间返回字符串
         */
        Util.prototype.calculationTimeToStr = function(millisecond, notZerofill){
        	var obj = this.calculationTimeByMillisecond(millisecond, notZerofill);
            if(obj.months){
                return '一个月以上';
            }else if(obj.days){
                return obj.days + '天' + (obj.hours?obj.hours + '小时':'');
            }else if(obj.hours){
                return obj.hours + '小时' + (obj.minutes?obj.minutes + '分钟':'');
            }else if(obj.minutes){
                return obj.minutes +  '分钟' + (obj.seconds?obj.seconds + '秒':'');
            }else{
                return obj.seconds + '秒';
            }
        };
        ...
        Util.prototype.everyTime = function (options) {
            if (!(options.key in Util.everyTime)) {
                ...
                var fn = function loop() {
                    Util.everyTime[options.key] = setTimeout(loop, options.time);
                    options.fn();
                }
                Util.everyTime[options.key] = setTimeout(fn, options.time);
            }
        };
         ...
         * 用于cookie存储值，所在的域
         */
        Util.prototype.getCookiePath = function(isOverall){
        	return '/';
        };
         ...
         * @constructor
         */
        Util.prototype.isInteger = function (){
            var result = arguments.length;
            for(var i = 0; i < arguments.length; i++){
                result = result && /^[0-9]+$/.test(arguments[i])
            }
            return !!result;
        };
        return new Util();
